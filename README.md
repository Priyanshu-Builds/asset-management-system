# 🏢 Vaultix — Enterprise Asset Management System
...
Vaultix is an enterprise-grade asset management system designed for organizations to track hardware, software, furniture, and vehicles throughout their lifecycle. It provides different dashboards and permissions based on user roles — administrators see system-wide analytics and manage resources, while employees view only their assigned assets and report issues.
...
- **Notifications System** — Dedicated notifications page with per-user read/unread tracking, mark-all-read functionality, and role-based notification filtering
- **Forgot Password** — Password reset flow via email with dedicated reset page
...
- **Backend Status Indicator** — Real-time health check with auto-retry for cold-start detection on Render
...
- **Orbital Ring Decorations** — Animated glowing rings with floating avatars and IT device icons on the landing and auth pages
- **Landing Page** — Full SaaS-style landing page with hero section, feature grid, role comparison cards, "How it Works" steps, logo bar, and footer
- **Micro-Animations** — Smooth transitions, hover effects, sparkle dots, and animated modals
- **Responsive Design** — Fully responsive from desktop to mobile with adaptive grid layouts
...
| cryptography | MySQL SSL connection support |
...
│
├── backend/
│   ├── app.py                    # Flask app, blueprint registration, CORS config
│   ├── config.py                 # DB URI, JWT secrets, connection pooling (env vars)
│   ├── models.py                 # 6 SQLAlchemy models
│   ├── seed.py                   # Populates database with demo data (uses DATABASE_URL)
│   ├── requirements.txt          # Python dependencies
│   ├── render.yaml               # Render deployment configuration
│   ├── .env.example              # Required environment variables reference
│   └── routes/
│       ├── auth.py               # POST /auth/login, /auth/register, /auth/forgot-password
...
│   │   ├── assets/avatars.js     # Avatar image imports
│   │   ├── components/
...
│   │   │   ├── AuthDecorations.jsx # Orbital rings, sparkles, floating items
│   │   │   └── BackendStatus.jsx # Health check indicator with auto-retry
│   │   ├── pages/
│   │   │   ├── LandingPage.jsx   # SaaS landing page with hero, features, roles
...
│   │   │   ├── Notifications.jsx # Full notifications page with read tracking
...
|---|---|---|---|
| **Admin** | admin@volkswagen.in | admin123 | Full analytics, all modules, user management |
| **IT Manager** | rahul@volkswagen.in | password123 | Full analytics, all modules except user management |
